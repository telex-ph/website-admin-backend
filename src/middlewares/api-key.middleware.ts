...
export const verifyApiKey = (req: Request, res: Response, next: NextFunction) => {
  ...
  // CONFIRMED: signs body-only (no timestamp in payload)
  // ============================================
  ...
  if (sightaiSignature && sightaiTimestamp) {
    console.log("🤖 [API-KEY] Sight AI webhook detected — verifying signature");

    if (!expectedSecret) {
      console.error("❌ [API-KEY] BLOG_API_KEY not set in environment variables");
      return res.status(500).json({ message: "Server misconfiguration" });
    }

    // ✅ CONFIRMED FORMAT: HMAC SHA256 of raw JSON body only
